...
public final class CSVFormat implements Serializable {
    ...
    private void validate() throws IllegalArgumentException {
        Validate.isTrue(!isLineBreak(delimiter), "The delimiter cannot be a line break");

        Validate.isTrue(quoteCharacter == null || delimiter != quoteCharacter,
                "The quoteChar character and the delimiter cannot be the same ('" + quoteCharacter + "')");

        Validate.isTrue(escapeCharacter == null || delimiter != escapeCharacter,
                "The escape character and the delimiter cannot be the same ('" + escapeCharacter + "')");

        Validate.isTrue(commentMarker == null || delimiter != commentMarker,
                "The comment start character and the delimiter cannot be the same ('" + commentMarker + "')");

        Validate.isTrue(quoteCharacter == null || !quoteCharacter.equals(commentMarker),
                "The comment start character and the quoteChar cannot be the same ('" + commentMarker + "')");

        Validate.isTrue(escapeCharacter == null || !escapeCharacter.equals(commentMarker),
                "The comment start and the escape character cannot be the same ('" + commentMarker + "')");

        Validate.isTrue(escapeCharacter != null || quoteMode != QuoteMode.NONE, "No quotes mode set but no escape character is set");

        // validate header
        if (header != null) {
            final Set<String> dupCheck = new HashSet<String>();
            for (final String hdr : header) {
                Validate.isTrue(dupCheck.add(hdr), "The header contains a duplicate entry: '" + hdr + "' in " + Arrays.toString(header));
            }
        }
    }
    ...
    public CSVFormat withCommentMarker(final Character commentMarker) {
        Validate.isTrue(!isLineBreak(commentMarker), "The comment start marker character cannot be a line break");
        return new CSVFormat(delimiter, quoteCharacter, quoteMode, commentMarker, escapeCharacter,
                ignoreSurroundingSpaces, ignoreEmptyLines, recordSeparator, nullString, headerComments, header,
                skipHeaderRecord, allowMissingColumnNames, ignoreHeaderCase, trim, trailingDelimiter);
    }
    ...
    public CSVFormat withEscape(final Character escape) {
        Validate.isTrue(!isLineBreak(escape), "The escape character cannot be a line break");
    ...
    public CSVFormat withQuote(final Character quoteChar) {
        Validate.isTrue(!isLineBreak(quoteChar), "The quoteChar cannot be a line break");
